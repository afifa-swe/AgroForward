          ...
          <div>
            <div class="quick-form-wrap">
              ...
              <template v-if="!quickSent">
                <div class="form-field"><label>Имя</label><input type="text" placeholder="Иван Петров"></div>
                <div class="form-row">
                  <div class="form-field"><label>Телефон</label><input type="tel" placeholder="+7 (000) 000-00-00"></div>
                  <div class="form-field"><label>Email</label><input type="email" placeholder="ivan@company.ru"></div>
                </div>
                <div class="form-field"><label>Тема</label><select><option>Стать партнёром</option><option>Запрос цен</option><option>Документация</option><option>Инвестиции</option><option>Другое</option></select></div>
                <div class="form-field"><label>Сообщение</label><textarea placeholder="Ваш вопрос или запрос…"></textarea></div>
                <button class="btn-p" style="width:100%" @click="quickSent = true">Отправить</button>
              </template>
      ...
      <div v-if="activeTab === 'form'" class="reveal">
        ...
        <div style="max-width:640px">
          <template v-if="!formSent">
            <div class="form-row">
              <div class="form-field"><label>Имя *</label><input type="text" placeholder="Иван"></div>
              <div class="form-field"><label>Фамилия *</label><input type="text" placeholder="Петров"></div>
            </div>
            <div class="form-field"><label>Компания</label><input type="text" placeholder="ООО «Название»"></div>
            <div class="form-row">
              <div class="form-field"><label>Email *</label><input type="email" placeholder="ivan@company.ru"></div>
              <div class="form-field"><label>Телефон</label><input type="tel" placeholder="+7 (000) 000-00-00"></div>
            </div>
            <div class="form-field"><label>Тема обращения *</label>
              <select><option value="">Выберите тему...</option><option>Новое партнёрство</option><option>Запрос цен</option><option>Документация</option><option>Инвестиции</option><option>Логистика</option><option>Жалоба</option><option>Другое</option></select>
            </div>
            <div class="form-field"><label>Сообщение *</label><textarea placeholder="Подробно опишите ваш запрос…" style="min-height:160px"></textarea></div>
            <div style="display:flex;align-items:center;gap:.8rem;margin-bottom:1.2rem">
              <input type="checkbox" id="agree" style="width:16px;height:16px;accent-color:var(--terra)">
            ...
            <button class="btn-p" @click="formSent = true">Отправить обращение</button>
          </template>
